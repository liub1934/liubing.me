---
toc: false
order: 35
layout: NavLayout
containerClass: nav-container
---

# AI 提示词

::: projects

```yaml
- icon: '@websites/stariu.svg'
  name: Stariu
  desc: AI绘画提示词生成工具。
  link: https://stariu.com/

- icon: '@websites/promptbase.png'
  name: PromptBase
  desc: AI绘画提示词市场。
  link: https://promptbase.com/

- icon: '@websites/prompthero.png'
  name: PromptHero
  desc: 通过 Stable Diffusion、Midjourney 等模型搜索数百万张 AI 艺术图像。
  link: https://prompthero.com/

- icon: '@websites/arthub.ico'
  name: Arthub
  desc: AI绘画提示词市场。
  link: https://arthub.ai/

- icon: '@websites/prompt.noonshot.ico'
  name: PromptBase
  desc: Midjourney提示此生成工具。
  link: https://prompt.noonshot.com/

- icon: '@websites/civitai.png'
  name: Civitai
  desc: 艺术共享平台，同时也提供提示此复制使用。
  link: https://civitai.com/

- icon: '@websites/civitai.png'
  name: Civitai
  desc: 模型共享平台，同时也提供提示此复制使用。
  link: https://civitai.com/

- icon: '@websites/lexica.ico'
  name: Lexica
  desc: Stable Diffusion搜索引擎。
  link: https://lexica.art/

- icon: '@websites/openart.png'
  name: OpenArt
  desc: 根据喜欢的图片复制提示词。
  link: https://openart.ai/discovery

- icon: '@websites/search.krea.ico'
  name: KREA
  desc: 面向所有人的生成视觉效果，获取创作灵感。
  link: https://search.krea.ai/

- icon: '@websites/aigallery.png'
  name: AI画廊
  desc: 输入中/英文描述生成关键词。
  link: https://www.aigallery.top/

- icon: '@websites/promptomania.png'
  name: promptoMANIA
  desc: 免费的提示生成器。
  link: https://promptomania.com/prompt-builder/

- icon: '@websites/kalos.png'
  name: KALOS
  desc: 强大的人工智能艺术展示和市场。
  link: https://kalos.art/

- icon: '@websites/originality.ai.png'
  name: Originality.ai
  desc: AI 提示词生成器。
  link: https://originality.ai/ai-prompt-generator
```

:::
