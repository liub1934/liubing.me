---
toc: false
layout: NavLayout
containerClass: nav-container
---

# 下载神器

::: projects

```yaml
- icon: 'https://api.liubing.me/favicon/internetdownloadmanager.com.png'
  name: IDM
  desc: Win系统下的神级下载器。
  link: https://internetdownloadmanager.com/

- icon: 'https://api.liubing.me/favicon/freedownloadmanager.org.png'
  name: FDM
  desc: 跨平台強大又现代的下载加速神器。
  link: https://freedownloadmanager.org/zh/

- icon: 'https://api.liubing.me/favicon/motrix.app.png'
  name: Motrix
  desc: 一款开源全能的下载工具。
  link: https://motrix.app/

- icon: '@websites/downie.png'
  name: Downie
  desc: Mac 上最简单省心的万能在线视频下载器。
  link: https://software.charliemonroe.net/downie/

- icon: 'https://api.liubing.me/favicon/qbittorrent.org.png'
  name: qBittorrent
  desc: 一个替代 μTorrent 的开源软件。
  link: https://qbittorrent.org/

- icon: 'https://api.liubing.me/favicon/transmissionbt.com.png'
  name: Transmission
  desc: 免费的 BitTorrent 客户端。
  link: https://transmissionbt.com/

- icon: 'https://api.liubing.me/favicon/gopeed.com.png'
  name: Gopeed
  desc: 一款现代化的开源下载器，支持多协议。
  link: https://gopeed.com/zh-CN
```

:::
