---
toc: false
layout: NavLayout
containerClass: nav-container
---

# 文档办公

::: projects

```yaml
- icon: '@websites/draw.io.ico'
  name: Draw.io
  desc: 开源免费的跨平台流程图绘图制图工具。
  link: https://app.diagrams.net/

- icon: '@websites/edrawsoft.ico'
  name: 亿图
  desc: 简单好用的流程图 / 思维导图制作工具。
  link: https://www.edrawsoft.cn/

- icon: '@websites/mubu.ico'
  name: 幕布
  desc: 极简大纲笔记，一键生成思维导图。
  link: https://mubu.com/

- icon: '@websites/yuque.png'
  name: 语雀
  desc: 全能云笔记文档知识库工具。
  link: https://www.yuque.com/

- icon: '@websites/feishu.ico'
  name: 飞书
  desc: 先进企业协作与管理平台，一站式无缝办公协作。
  link: https://www.feishu.cn/

- icon: '@websites/flomoapp.png'
  name: 浮墨笔记
  desc: 不仅能帮你快速记录，还能帮你更好回顾。
  link: https://flomoapp.com/

- icon: '@websites/notion.png'
  name: Notion
  desc: 大名鼎鼎的笔记软件。
  link: https://www.notion.so/

- icon: '@websites/onenote.png'
  name: OneNote
  desc: 微软出品的跨平台优秀免费笔记软件/记事/资料收集整理工具。
  link: https://www.onenote.com/

- icon: '@websites/yinxiang.ico'
  name: 印象笔记
  desc: 一款在全球范围内最受欢迎的笔记软件 (知识管理工具)。
  link: https://www.yinxiang.com/

- icon: '@websites/youdao.ico'
  name: 有道云笔记
  desc: 网易旗下笔记应用，定位对标印象笔记。
  link: https://note.youdao.com/

- icon: '@websites/siyuan.png'
  name: 思源笔记
  desc: 一款本地优先的个人知识管理系统，支持完全离线使用。
  link: https://b3log.org/siyuan/

- icon: '@websites/wiz.png'
  name: 为知笔记
  desc: 跨平台个人知识管理软件。
  link: https://www.wiz.cn/zh-cn/

- icon: '@websites/joplinapp.ico'
  name: Joplin
  desc: 以知识释放创造力，从个人笔记到团队知识库，构建更强大的知识图谱。
  link: https://joplinapp.org/

- icon: '@websites/zoho.ico'
  name: Zoho Notebook
  desc: 精美卡片式笔记，可用于记笔记、创建任务清单、记录语音与视频等。
  link: https://www.zoho.com/

- icon: '@websites/yank-note.ico'
  name: Yank Note
  desc: 一款强大可扩展的 Markdown 编辑器，支持运行代码片段。
  link: https://yank-note.com/zh-CN
```

:::
