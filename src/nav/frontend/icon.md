---
toc: false
order: 60
layout: NavLayout
containerClass: nav-container
---

# 字体图标

::: projects

```yaml
- icon: 'https://api.iowen.cn/favicon/iconfont.cn.png'
  name: iconfont
  desc: 阿里巴巴矢量图标库。
  link: https://iconfont.cn/

- icon: 'https://api.iowen.cn/favicon/oceanengine.com.png'
  name: IconPark
  desc: 字节跳动出品的高质量开源图标库。
  link: https://iconpark.oceanengine.com/

- icon: 'https://api.iowen.cn/favicon/fontawesome.com.png'
  name: Font Awesome
  desc: Font Awesome 是一款很流行的字体图标工具。
  link: https://fontawesome.com/

- icon: 'https://api.iowen.cn/favicon/iconify.design.png'
  name: Iconify
  desc: 免费开源的图标库，超过150k+图标自由选择。
  link: https://iconify.design/

- icon: 'https://api.iowen.cn/favicon/icons8.com.png'
  name: Icons8
  desc: 图标、插图、照片、音乐和设计工具。
  link: https://icons8.com/

- icon: 'https://api.iowen.cn/favicon/flaticon.com.png'
  name: Flaticon
  desc: 超过 920 万个矢量图标和贴纸。
  link: https://flaticon.com/

- icon: 'https://api.iowen.cn/favicon/lordicon.com.png'
  name: Lordicon
  desc: 一个强大的精心制作的动画图标库，支持8,900+动画图标。
  link: https://lordicon.com/

- icon: 'https://api.iowen.cn/favicon/xicons.org.png'
  name: xicons
  desc: 整合了N多组件库的Icon图标，提供Vue/React组件。
  link: https://xicons.org/#/zh-CN
```

:::
